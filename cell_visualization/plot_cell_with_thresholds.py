...
from tms_probability.probability_analysis import get_thresholds, get_results
from tms_probability.plotting import get_threshold_map_by_cell, get_threshold_diff_by_cell
from tms_probability.sim_control import TMS_sim
from tms_probability.threshold_sim import detect_spike
from extracellular_stim_tools.netpyne_extracellular import SingleExtracellular, calculate_segments_centers
...
def overlay_cell_and_thresholds(threshold_fname, cell_name, radius, condition=None, init_site_direction=None, thresh_buffer=0):
    ...
    if type(threshold_fname) == str:
        results = reach_out_for_func(get_results, threshold_fname)
        cell_data = results['threshold_map']
        data_map_by_cell = get_threshold_map_by_cell(cell_data)
        plot_threshold_sphere_map(data_map_by_cell, cell_name, ax, radius=radius, condition=condition)
        ...
        results_0 = reach_out_for_func(get_results, threshold_fname[0])
        results_1 = reach_out_for_func(get_results, threshold_fname[1])
        ...
        plot_threshold_diff_sphere_map(data_map_by_cell, cell_name, ax, radius=radius, conditions=condition)
    ...
    if init_site_direction == None: plot_cell(cell_name, ax)
        ...
        if sim_direction != None:
            for res in results_list:
                ...
                ecs = TMS_sim(cell_name, tms_params, suppress_logs=True)
                if detect_spike(ecs):
                    init_sec_id = int(ecs.action_potentials_recording_ids[0])
                    print(ecs.stim_cell.section_list[init_sec_id].name())
                    non_init_sec_pts = list(calculate_segments_centers(ecs.stim_cell.section_list))
