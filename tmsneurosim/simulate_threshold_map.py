...
from tmsneurosim.nrn.simulation.threshold_uniform_gradient_simulation import (
    ThresholdUniformGradientSimulation,
)

progress_counter = multiprocessing.Array("i", [])


def simulate_gradient_threshold_map(
    neuron_cell: NeuronCell,
    waveform_type: WaveformType,
    theta_step,
    phi_step,
    e_gradient_min_percentage=None,
    e_gradient_max_percentage=None,
    number_of_e_gradients=None,
    theta_gradient_min_degree=None,
    theta_gradient_max_degree=None,
    number_of_theta_degree=None,
    phi_gradient_min_degree=None,
    phi_gradient_max_degree=None,
    ...
    """
    if (
        e_gradient_min_percentage is None
        or e_gradient_max_percentage is None
        or number_of_e_gradients is None
    ):
    ...
    if (
        phi_gradient_min_degree is None
        or phi_gradient_max_degree is None
        or number_of_phi_degree is None
    ):
    ...
    if (
        theta_gradient_min_degree is None
        or theta_gradient_max_degree is None
        or number_of_theta_degree is None
    ):
    ...
    e_field_list = []
    for theta_gradient in np.linspace(
        theta_gradient_min_degree, theta_gradient_max_degree, number_of_theta_degree
    ):
        for phi_gradient in np.linspace(
            phi_gradient_min_degree, phi_gradient_max_degree, number_of_phi_degree
        ):
            for e_magnitude_gradient in np.linspace(
                e_gradient_min_percentage,
                e_gradient_max_percentage,
    ...
    parameters_per_process = [
        e_field_list[i : i + chunk_size]
        for i in range(0, len(e_field_list), chunk_size)
    ]
    ...
    with multiprocessing.Pool(processes=process_count) as pool:
        ...
        all_done = False
        with tqdm(
            total=len(e_field_list),
            unit="sims",
            desc=f"{neuron_cell.__class__.__name__}_{neuron_cell.morphology_id}",
        ) as pbar:
            while not all_done:
...
def simulate_uniform_threshold(
    neuron_cell: NeuronCell,
    waveform_type: WaveformType,
    e_field_list: [Tuple[float, float, float, float]],
    index=None,
):
    """
    ...
    for i, (
        theta,
        phi,
        relative_e_gradient_per_mm,
        phi_gradient_per_mm,
        theta_gradient_per_mm,
    ) in enumerate(e_field_list):
        simulation.apply_e_field(
            theta,
            phi,
            relative_e_gradient_per_mm,
            phi_gradient_per_mm,
