...
def crop_box(
    mesh: simnibs.Msh, roi: [float], keep_elements: bool = False
) -> simnibs.Msh:
    ...
    if keep_elements:
        ...
        elements_to_keep = (
            np.where(
                np.all(
                    np.isin(mesh.elm.node_number_list, node_keep_indexes).reshape(
                        -1, 4
                    ),
                    axis=1,
                )
...
class CorticalLayer:
    ...
    def __init__(
        ...
        roi: [float] = None,
        depth: float = None,
        elements_per_square_mm: float = None,
        path=None,
        ...
        if path is not None:
            ...
            self.volumetric_mesh = crop_box(
                volumetric_mesh, roi + np.tile([-5, 5], 3), True
            )
            self.surface = None
        ...
        if elements_per_square_mm is None:
            ...
            self.elements = self.get_evenly_spaced_element_subset(
                elements_per_square_mm
            )
            logging.info("Finished element selection")
            logging.info(
                f"Selected {len(self.elements)}/{self.surface.elm.nr} elements"
        ...
        for elm_field in self.volumetric_mesh.elmdata:
            for tag in np.unique(
                self.volumetric_mesh.elm.tag1[self.volumetric_mesh.elm.tetrahedra - 1]
            ):
                self.node_fields[(tag, elm_field.field_name)] = (
                    self.volumetric_mesh.crop_mesh(tags=[tag])
                    .field[elm_field.field_name]
                    .elm_data2node_data()
                )
    ...
    def generate_layer(self, depth, roi):
        ...
        point_tissue_tag = self.volumetric_mesh.find_tetrahedron_with_points(
            grid_points, compute_baricentric=False
        )
        ...
        points_outside = np.where(
            (point_tissue_tag != WHITE_MATTER) & (point_tissue_tag != GRAY_MATTER)
        )[0]
        points_inside_gray_matter = np.where(point_tissue_tag == GRAY_MATTER)[0]

        roi_points = np.stack(
            np.meshgrid(outer_roi[0:2], outer_roi[2:4], outer_roi[4:6], indexing="ij"),
            axis=-1,
        ).reshape(-1, 3)
        roi_tissue_tag = self.volumetric_mesh.elm.tag1[
            self.volumetric_mesh.find_tetrahedron_with_points(
                roi_points, compute_baricentric=False
            )
        ]
        ...
        wm_surface = self.volumetric_mesh.crop_mesh(
            WHITE_MATTER_SURFACE
        ).nodes.node_coord
        gm_surface = self.volumetric_mesh.crop_mesh(
            GRAY_MATTER_SURFACE
        ).nodes.node_coord
        ...
        gray_matter_interpolation = interpolation(
            grid_points[points_inside_gray_matter]
        )
        ...
        inside_gray_matter_x = (
            points_inside_gray_matter / (len(grid_z) * len(grid_y))
        ).astype(int)
        inside_gray_matter_y = (
            (points_inside_gray_matter / len(grid_z)) % len(grid_y)
        ).astype(int)
        inside_gray_matter_z = (points_inside_gray_matter % len(grid_z)).astype(int)
        volume_data[
            (inside_gray_matter_x, inside_gray_matter_y, inside_gray_matter_z)
        ] = gray_matter_interpolation
        ...
        self.surface.nodes.node_coord = self.surface.nodes.node_coord + [
            (outer_roi[0]),
            (outer_roi[2]),
            (outer_roi[4]),
        ]
        ...
        logging.info(
            f"Created layer ({np.sum(self.surface.elements_volumes_and_areas().value)} mm²) "
    ...
    def add_e_field_at_triangle_centers_field(self):
        ...
        self.surface.add_element_field(
            np.array([np.linalg.norm(v) for v in e_field_at_centers]),
            "E-Field_Magnitude",
        )
    ...
    def add_e_field_gradient_selected_elements_field(self, above, below, prefix=""):
        ...
        e_field_vector_above = self.volumetric_mesh.field["E"].interpolate_scattered(
            point_above, out_fill=0
        )
        e_field_vector_below = self.volumetric_mesh.field["E"].interpolate_scattered(
            point_below, out_fill=0
        )
        ...
        self.add_selected_elements_field(
            e_field_gradient_per_mm, f"{prefix}_Gradient_per_mm"
        )

    def add_position_tag_field(
        self, prefix="", cell_height=1, above=None, below=None, cell: NeuronCell = None
    ):
        """
        ...
        tags_above = self.volumetric_mesh.elm.tag1[
            self.volumetric_mesh.find_tetrahedron_with_points(
                point_above, compute_baricentric=False
            )
        ]
        tags_below = self.volumetric_mesh.elm.tag1[
            self.volumetric_mesh.find_tetrahedron_with_points(
                point_below, compute_baricentric=False
            )
        ]
    ...
    def add_e_field_gradient_between_wm_gm_field(self):
        ...
        gray_matter_surface: simnibs.Msh = self.volumetric_mesh.crop_mesh(
            GRAY_MATTER_SURFACE
        )
        white_matter_surface: simnibs.Msh = self.volumetric_mesh.crop_mesh(
            WHITE_MATTER_SURFACE
        )

        _, closest_gm_indexes, gm_distance = gray_matter_surface.find_closest_element(
            centers, return_index=True, return_distance=True
        )
        _, closest_wm_indexes, wm_distance = white_matter_surface.find_closest_element(
            centers, return_index=True, return_distance=True
        )

        gm_centers = (
            gray_matter_surface.elements_baricenters()[closest_gm_indexes]
            - gray_matter_surface.triangle_normals()[closest_gm_indexes] * 0.001
        )
        wm_centers = (
            white_matter_surface.elements_baricenters()[closest_wm_indexes]
            + white_matter_surface.triangle_normals()[closest_wm_indexes] * 0.001
        )

        e_field_above = self.volumetric_mesh.field["E"].interpolate_scattered(
            gm_centers, out_fill=0
        )
        e_field_below = self.volumetric_mesh.field["E"].interpolate_scattered(
            wm_centers, out_fill=0
        )

        e_field_above_mag = np.array([np.linalg.norm(v) for v in e_field_above])[
            :, None
        ].flatten()
        e_field_below_mag = np.array([np.linalg.norm(v) for v in e_field_below])[
            :, None
        ].flatten()
    ...
    def add_delta_e_mag_between_wm_gm_field(self):
        ...
        layer_gm_distance = np.linalg.norm(
            layer_centers - gm_intersec_pts, ord=2, axis=1
        )
        layer_wm_distance = np.linalg.norm(
            layer_centers - wm_intersec_pts, ord=2, axis=1
        )

        raycast_error_nodes_gm = np.argwhere(layer_gm_distance > 2)  # mm
        raycast_error_nodes_wm = np.argwhere(layer_wm_distance > 2)  # mm

        closest_gm_nodes, _ = gray_matter_surface.nodes.find_closest_node(
            layer_centers[raycast_error_nodes_gm], return_index=True
        )
        closest_wm_nodes, _ = white_matter_surface.nodes.find_closest_node(
            layer_centers[raycast_error_nodes_wm], return_index=True
        )
        ...
        associated_white_matter_points = (
            wm_intersec_pts
            - np.multiply(
        ...
        e_field_above = self.volumetric_mesh.field["E"].interpolate_scattered(
            associated_gray_matter_points, out_fill=0
        )
        e_field_below = self.volumetric_mesh.field["E"].interpolate_scattered(
            associated_white_matter_points, out_fill=0
        )
    ...
    def get_evenly_spaced_element_subset(self, elements_per_square_mm):
        ...
        for element_index in tqdm(range(self.surface.elm.nr)):
            ...
            distances_square = (
                (selected_elements_centers[:, 0] - element_center[0]) ** 2
    ...
    def add_nearest_interpolation_field(self, values, name):
        ...
        for element_index in tqdm(range(self.surface.elm.nr)):
            ...
            element_center = centers[element_index]
            distances_square = (
                (elements_centers[:, 0] - element_center[0]) ** 2
                + (elements_centers[:, 1] - element_center[1]) ** 2
                + (elements_centers[:, 2] - element_center[2]) ** 2
            )
            nearest_interpolation_values[element_index] = element_values[
                np.argmin(distances_square)
            ]
    ...
    def interpolate_scattered_cashed(
        self, points, elm_field: str, out_fill=np.nan, get_tetrahedron_tags=False
    ):
        """$SimNIBS$"""
        ...
        th_with_points, bar = self.volumetric_mesh.find_tetrahedron_with_points(
        ...
        if np.any(inside):
            ...
            for t in np.unique(sorted_tag):
                ...
                msh_with_t = self.volumetric_mesh.elm.elm_number[
                    self.volumetric_mesh.elm.tag1 == t
                ]
