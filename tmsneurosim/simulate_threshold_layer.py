...
def simulate_combined_threshold_layer(
    ...
    for layer_result, (cell, i) in zip(layer_results, layer_meta_info):
        layer.add_selected_elements_field(
            np.array(layer_result[0]),
            f"{cell.__class__.__name__}_{cell.morphology_id}__{i}",
        )
        layer.add_selected_elements_field(
            np.array(layer_result[1]),
    ...
    if len(layer_results) > 1:
        ...
        layer.add_selected_elements_field(
            np.nanmedian(layer_thresholds, axis=0), "Median_Threshold"
        )
        layer.add_selected_elements_field(
            np.nanmean(layer_thresholds, axis=0), "Mean_Threshold"
        )
        layer.add_nearest_interpolation_field(
            np.nanmedian(layer_thresholds, axis=0), "Median_Threshold_Visual"
        )
        layer.add_nearest_interpolation_field(
            np.nanmean(layer_thresholds, axis=0), "Mean_Threshold_Visual"
        )

    return layer.surface


def simulate_threshold_layer(
    layer: CorticalLayer,
    cell: NeuronCell,
    waveform_type: WaveformType,
    azimuthal_rotations: NDArray[np.float64],
    rotation_count,
    process_count: int,
    ...
    logging.info(
        f"Started {cell.__class__.__name__}_{cell.morphology_id} ({rotation_count})"
    )
    thresholds = []
    tags = []
    with multiprocessing.Pool(processes=process_count) as pool:
        sims = []
        for i, (directions, positions, azimuthal_rotations) in enumerate(
            zip(
                directions_per_process,
                positions_per_process,
                azimuthal_rotations_per_process,
        ...
        with tqdm(
            ...
            unit="sims",
            desc=f"{cell.__class__.__name__}_{cell.morphology_id} ({rotation_count})",
        ) as pbar:
            while not all_done:
    ...
    logging.info(
        f"Finished {cell.__class__.__name__}_{cell.morphology_id} ({rotation_count})"
    )

    return thresholds, tags


def simulate_cell_thresholds(
    cell: NeuronCell,
    waveform_type: WaveformType,
    ...
    thresholds = []
    tags = []
    for i, (direction, position, azimuthal_rotation) in enumerate(
        zip(directions, positions, azimuthal_rotations)
    ):
        threshold_and_tag = simulate_cell_threshold(
            cell,
            simulation,
            segment_points_mm,
            direction,
...
def simulate_cell_threshold(
    ...
    transformed_cell_coordinates = (
        rotation.apply(azimuthal_rotation.apply(transformed_cell_coordinates))
        + position
    )
    ...
    transformed_e_field = azimuthal_rotation.inv().apply(
        rotation.inv().apply(e_field_at_cell)
    )

    simulation.apply_e_field(transformed_e_field)
    return simulation.find_threshold_factor(), int(
        "".join(map(str, np.unique(tetrahedron_tags)))[::-1]
    )
