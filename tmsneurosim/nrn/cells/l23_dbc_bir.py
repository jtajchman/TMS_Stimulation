...
class L23_DBC_bIR(NeuronCell):
    def __init__(
        ...
        super().__init__(
            str(
                list(
                    pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L23_DBC_bIR/morphology/').resolve()))
    ...
    def get_morphology_ids():
        return sorted(
            [
                int(f.name)
                for f in pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L23_DBC_bIR/morphology/').resolve())).iterdir()
                if f.is_dir()
            ]
        )
    ...
    def apply_biophysics(self):
        ...
        for dend_section in self.dend:
            for dend_segment in dend_section.allseg():
                ...
                dend_segment.gkbar_StochKv = (0.0 * dend_segment_distance + 1.0)*0.290744
        ...
        for apic_section in self.apic:
            for apic_segment in apic_section.allseg():
                ...
                apic_segment.gkbar_StochKv = (0.0 * apic_segment_distance + 1.0)*0.290744
        ...
        for soma_section in self.soma:
            for soma_segment in soma_section.allseg():
                ...
                soma_segment.gkbar_StochKv = (0.0 * soma_segment_distance + 1.0)*0.658298
