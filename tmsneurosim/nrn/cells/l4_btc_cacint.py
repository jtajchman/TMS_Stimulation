...
class L4_BTC_cACint(NeuronCell):
    def __init__(
        self,
        morphology_id,
        ...
        super().__init__(
            str(
                list(
                    pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L4_BTC_cACint/morphology/').resolve()))
    ...
    def get_morphology_ids():
        return sorted(
            [
                int(f.name)
                for f in pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L4_BTC_cACint/morphology/').resolve())).iterdir()
                if f.is_dir()
            ]
        )
