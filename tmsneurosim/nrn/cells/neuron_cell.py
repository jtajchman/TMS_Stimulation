...
class NeuronCell:
    ...
    @staticmethod
    def _scale_section_point_diameters(scale_factor: float, sections: list[nrn.Section]) -> None:
    ...
    def _calculate_myelin_section_counts(self, axon_without_root: list[nrn.Section]) -> tuple[list[int], int, int]:
    ...
    def _interpolate_point_between(distance: float, section_end: nrn.Section, section_end_index: int,
                                   ...
                                   section_start_index: int) -> tuple[float, float, float, float]:
    ...
    def get_segment_coordinates(self) -> np.ndarray[float]:
