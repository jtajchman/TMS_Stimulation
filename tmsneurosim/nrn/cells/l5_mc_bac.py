...
class L5_MC_bAC(NeuronCell):
    def __init__(
        ...
        super().__init__(
            str(
                list(
                    pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L5_MC_bAC/morphology/').resolve()))
    ...
    def get_morphology_ids():
        return sorted(
            [
                int(f.name)
                for f in pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L5_MC_bAC/morphology/').resolve())).iterdir()
                if f.is_dir()
            ]
        )
