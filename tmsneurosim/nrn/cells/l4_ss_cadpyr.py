...
class L4_SS_cADpyr(NeuronCell):
    def __init__(self, morphology_id, modification_parameters: CellModificationParameters = None, variation_seed=None):
        super().__init__(str(list(pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L4_SS_cADpyr/morphology/').resolve())).joinpath(f'{morphology_id}').iterdir())[0]),
                         morphology_id, modification_parameters, variation_seed)

    @staticmethod
    def get_morphology_ids():
        return sorted([int(f.name) for f in pathlib.Path(str(pathlib.Path(tmsneurosim.nrn.__file__).parent.joinpath('cells/cells_hoc/L4_SS_cADpyr/morphology/').resolve())).iterdir() if f.is_dir()])
    ...
    def apply_biophysics(self):
        ...
        for apic_section in self.apic:
            for apic_segment in apic_section.allseg():
                ...
                apic_segment.gNaTs2_tbar_NaTs2_t = (0.0 * apic_segment_distance + 1.0)*0.022874
                apic_segment.gSKv3_1bar_SKv3_1 = (0.0 * apic_segment_distance + 1.0)*0.039967
        ...
        for axon_section in self.axon:
            for axon_segment in axon_section.allseg():
                ...
                axon_segment.gNaTa_tbar_NaTa_t = (0.0 * axon_segment_distance + 1.0)*3.999904
                axon_segment.gK_Tstbar_K_Tst = (0.0 * axon_segment_distance + 1.0)*0.071166
                axon_segment.gamma_CaDynamics_E2 = (0.0 * axon_segment_distance + 1.0)*0.000502
                axon_segment.gNap_Et2bar_Nap_Et2 = (0.0 * axon_segment_distance + 1.0)*0.000056
                axon_segment.gSK_E2bar_SK_E2 = (0.0 * axon_segment_distance + 1.0)*0.097244
                axon_segment.gCa_HVAbar_Ca_HVA = (0.0 * axon_segment_distance + 1.0)*0.000860
                axon_segment.gK_Pstbar_K_Pst = (0.0 * axon_segment_distance + 1.0)*0.428834
                axon_segment.gSKv3_1bar_SKv3_1 = (0.0 * axon_segment_distance + 1.0)*0.004779
                axon_segment.decay_CaDynamics_E2 = (0.0 * axon_segment_distance + 1.0)*179.044149
                axon_segment.gCa_LVAstbar_Ca_LVAst = (0.0 * axon_segment_distance + 1.0)*0.001612
        for soma_section in self.soma:
            for soma_segment in soma_section.allseg():
                soma_segment_distance = h.distance(self.soma[0](0), soma_segment)
                soma_segment.gamma_CaDynamics_E2 = (0.0 * soma_segment_distance + 1.0)*0.002253
                soma_segment.gSKv3_1bar_SKv3_1 = (0.0 * soma_segment_distance + 1.0)*0.283745
                soma_segment.gSK_E2bar_SK_E2 = (0.0 * soma_segment_distance + 1.0)*0.002971
                soma_segment.gCa_HVAbar_Ca_HVA = (0.0 * soma_segment_distance + 1.0)*0.000379
                soma_segment.gNaTs2_tbar_NaTs2_t = (0.0 * soma_segment_distance + 1.0)*0.999812
                soma_segment.gIhbar_Ih = (0.0 * soma_segment_distance + 1.0)*0.000080
                soma_segment.decay_CaDynamics_E2 = (0.0 * soma_segment_distance + 1.0)*739.416497
                soma_segment.gCa_LVAstbar_Ca_LVAst = (0.0 * soma_segment_distance + 1.0)*0.006868
