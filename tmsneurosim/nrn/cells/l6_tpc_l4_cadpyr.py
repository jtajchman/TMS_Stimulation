                ...
                list(
                    pathlib.Path(
                        str(
                            ...
                            .parent.joinpath(
                                "cells/cells_hoc/L6_TPC_L4_cADpyr/morphology/"
                            )
                            .resolve()
                        )
                    )
        ...
        return sorted(
            [
                ...
                for f in pathlib.Path(
                    str(
                        pathlib.Path(tmsneurosim.nrn.__file__)
                        .parent.joinpath("cells/cells_hoc/L6_TPC_L4_cADpyr/morphology/")
                        .resolve()
                    )
                ).iterdir()
                if f.is_dir()
    ...
    def apply_biophysics(self):
        ...
        for apic_section in self.apic:
            for apic_segment in apic_section.allseg():
                ...
                apic_segment.gIhbar_Ih = (
                    -0.869600
                    + 2.087000 * math.exp((apic_segment_distance - 0.000000) * 0.003100)
                ) * 0.000080
        ...
        for axon_section in self.axon:
            for axon_segment in axon_section.allseg():
                ...
                axon_segment.gCa_HVAbar_Ca_HVA = (
                    0.0 * axon_segment_distance + 1.0
                ) * 0.000684
        ...
        for soma_section in self.soma:
            for soma_segment in soma_section.allseg():
                ...
                soma_segment.gCa_HVAbar_Ca_HVA = (
                    0.0 * soma_segment_distance + 1.0
                ) * 0.000459
