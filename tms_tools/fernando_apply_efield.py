...
def make_extracellular_stimuli(params, cell, secList):
    ...
    ext_field_scalar = np.vectorize(
        lambda x, y, z: (1 - decay_constant)  # um coordinates
        ** np.dot(np.array([x, y, z]) / 1000, field_direction)
    )
    ...
    v_cell_ext[:, :] = ext_field_scalar(x, y, z).reshape(totnsegs, 1) * np.array(
        pulse
    ).reshape(1, n_tsteps)
...
def make_tms_stim(net, params):
    ...
    for cell in net.cells:
        secList = [
            secs for secs in cell.secs.keys() if "pt3d" in cell.secs[secs]["geom"]
        ]
