...
# From tmseffects/tms_networks/core/codes/neuron_routines.py
def traverse_topology(section, section_list=None):
        ''' Returns a depth-first list of the sections in the tree starting at the given root section. '''
    ...
    for sec in section_list:
        ...
        for seg in sec:
            ...
            # if the segment is the first of the section:
                ...
                # displacement vector
                seg_disp = centers[len(sec_names)][segment_in_section_index] \
                    - centers[previous_sec_id][-1]      # Center of current segment - center of last segment of previous section (displacement)
                E_p = E_vectors[previous_sec_id][-1]      # Normalized E-field at last segment of previous section
                
                # get the quasipotential of the
                # previous section's last segment.
                phi_p = quasipotentials[previous_sec_id][-1]        # Quasipotential at last segment of previous section
            ...
            E_c = E_vectors[len(sec_names)][segment_in_section_index]     # Normalized E-field at current segment
...
def set_E_field(E_field_params: dict, section_list):
    ...
    for sec_segs in segments_centers:
        ...
        for seg_center in sec_segs:
            ext_field_scalar = (1-sigma) ** np.dot(np.array(seg_center)/1000, decay_dir)
    ...
    quasipotentials = calculate_cell_quasipotentials(E_vectors, segments_centers, section_list)
...
def set_stimulation(cell: CompartCell, params: dict):
    efield, time = get_efield(freq=params['frequency'], 
    ...
    section_list = traverse_topology(cell.secs['soma_0']['hObj'])
    ...
    E_field_params = {'field_direction': params['field_direction'],
                      'sigma': params['sigma'],
                      'decay_dir': params['decay_dir']}

    v_segments = set_E_field(E_field_params, section_list) # Sets v_segments as a list of quasipotentials corresponding to each segment in the cell
