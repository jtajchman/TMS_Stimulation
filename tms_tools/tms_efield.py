...
class pattern:
    def __init__(
        ...
        if interval == None:
            if p_onset_interval == None:
                if set_freq == None:
                    ...
                    if set_freq > 1 / width:
                        raise ValueError(
                            f"Set freq {set_freq} Hz must be <= 1/(pulse width) {1/width} Hz"
                        )
                    interval = 1 / set_freq - width
            else:
                if p_onset_interval < width:
                    raise ValueError(
                        f"Pulse onset interval {p_onset_interval} must be >= pulse width {width}"
                    )
                interval = p_onset_interval - width
        if interval < 0:
            raise ValueError(f"Pulse interval of pattern {interval} must be >= 0")
        self.rd = rd
        ...
        if self.pattern not in ["Single", "TBS", "Unique"]:
            raise ValueError(
                "Pulse pattern must be categorized as either 'Single', 'TBS', or 'Unique'"
            )
        if self.count == None:
            raise ValueError("Pulse count of pattern must be defined")
        elif self.count < 1:
            raise ValueError("Pulse count of pattern must be >= 1")
...
def efield(
    ...
    p_onset_interval = (
        pat.p_onset_interval
    )  # Duration of interval between the onset of pulses in a set
    ...
    set_width = (
        count - 1
    ) * inter_p_interval + count * pwidth  # Duration of one set of pulses
    set_onset_interval = (
        1 / freq
    )  # Duration of interval between the onset of sets of pulses
    inter_set_interval = (
        set_onset_interval - set_width
    )  # Duration of interval between sets of pulses
    ...
    if inter_set_interval < 0:
        raise ValueError(
            f"Duration of pulse set [{set_width} s] must be <= interval between pulse onset (1/frequency) [{set_onset_interval} s]"
        )
...
def get_efield(
    ...
    wav, time = efield(
        ...
        ef_amp=ef_amp,
        pat=pattern(
            pshape=shape(shape=pshape, width=width / 1e3),  # Convert from ms to s
            pattern=pat,
            count=count,
            interval=interval,
