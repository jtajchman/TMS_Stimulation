...
class Shape:
    def __init__(
        ...
        self.shape = shape
        self.pulse_width_ms = pulse_width_ms    # Defined pulse width in ms
        self.pulse_duration_ms = pulse_width_ms # Total duration of the pulse in ms (different from pulse width for classical pulses)

        # Define efield waveform shape normalized to a value of 1; scaling comes later        
        if self.shape == "Ideal_Sine":
            sin_freq_kHz = 2 * pi / self.pulse_width_ms # Angular frequency of sine wave
            self.efield_waveform = lambda t: np.cos(t * sin_freq_kHz) # Sinusoidal TMS pulses have cosinusoidal E-field waveforms
            self.pulse_type = "Ideal"
        elif self.shape == "Ideal_Square":
            self.efield_waveform = lambda t: np.ones_like(t) # Constant value of 1
            self.pulse_type = "Ideal"
        
        # Classical TMS pulse shapes recorded from a real coil interpret the pulse width as the duration of the first positive phase
        elif self.shape == "Biphasic":
            self.efield_waveform = coil_recording('b', self.pulse_width_ms)
            self.pulse_type = "Classical"
            ...
            raise ValueError(f"Pulse shape [{self.shape}] must be one of: \"Ideal_Sine\" \"Ideal_Square\" \"Biphasic\" \"Half-Sine\" \"Monophasic\"")
        
        if self.pulse_type == "Classical":
            # Pulse duration is scaled relative to the pulse width
            self.pulse_duration_ms = self.pulse_width_ms * (0.7374/0.075)
...
def coil_recording(shape_char, pulse_width_ms):
    ...
    pos_phase = 0.075 # Duration in ms of the positive phase of the pulse
    width_scalar = pulse_width_ms/pos_phase # Scale the time points to the desired pulse width
...
class Pattern:
    def __init__(
        ...
        pulse_duration_ms = self.pulse_shape.pulse_duration_ms
        ...
        if pulse_interval_within_burst_ms == None:
            if pulse_onset_interval_within_burst_ms == None:
                if pulse_freq_within_burst_Hz == None:
                    ...
                    if pulse_freq_within_burst_Hz > 1/pulse_duration_ms * kHz: # Comparison in Hz
                        raise ValueError(
                            f"pulse_freq_within_burst_Hz [{pulse_freq_within_burst_Hz}] must be <= 1/(pulse_duration_ms) [{1/pulse_duration_ms * kHz}]"
                        )
                    pulse_interval_within_burst_ms = 1/pulse_freq_within_burst_Hz * s - pulse_duration_ms
            else:
                if pulse_onset_interval_within_burst_ms < pulse_duration_ms:
                    raise ValueError(
                        f"pulse_onset_interval_within_burst_ms [{pulse_onset_interval_within_burst_ms}] must be >= pulse_duration_ms [{pulse_duration_ms}]"
                    )
                pulse_interval_within_burst_ms = pulse_onset_interval_within_burst_ms - pulse_duration_ms
        ...
        self.pulse_onset_interval_within_burst_ms = self.pulse_interval_within_burst_ms + pulse_duration_ms
...
def generate_efield(
    ...
    efield_waveform = pat.pulse_shape.efield_waveform  # E-field pulse function
    pulse_duration_ms = pat.pulse_shape.pulse_duration_ms  # Total duration of one pulse
    ...
    burst_width_ms = round(num_pulses_per_burst * pulse_onset_interval_within_burst_ms - inter_p_interval_ms, rd) # Duration of one burst of pulses (time of intervals + time of pulses)
    ...
    if burst_freq_Hz != None:
        ...
        burst_onset_interval_ms = simulation_duration_ms
        inter_burst_interval_ms = simulation_duration_ms
    ...
    ## Construct waveform of electric field

    # Initialize variables for waveform construction
    time = [] # Points in time
    wav = []  # E-field waveform for each point in time
    ...
    # Build pulse_start_times_ms (each pulse within a burst)
    ...
    # High resulution period of pulse
    if pat.pulse_shape.pulse_type == "Classical":
        high_resolution_time_dict = {"Biphasic":0.325/0.7374, "Half-Sine":0.175/0.7374, "Monophasic":0.2/0.7374} # Proportion of duration for high temporal resolution required to capture the pulse shapes
        hi_res_width_ms = round(high_resolution_time_dict[pat.pulse_shape.shape] * pulse_duration_ms, rd) # Duration in ms of high temporal resolution required to capture the pulse shapes
    else: # Ideal pulse shapes
        hi_res_width_ms = pulse_duration_ms
    # print(f"hi_res_width_ms: {hi_res_width_ms}")
    # print(f"pulse_duration_ms: {pulse_duration_ms}")

    # Build pulse_hi_res_end_times_ms to know when to switch to low resolution
    pulse_hi_res_end_times_ms = [round(start_time + hi_res_width_ms, rd) for start_time in pulse_start_times_ms]
    for ind, pulse_end_time in reversed(list(enumerate(pulse_hi_res_end_times_ms))): # Reversed so that pop(ind) does not cause index shift
        if pulse_end_time in pulse_start_times_ms:
            pulse_hi_res_end_times_ms.pop(ind)

    def get_t(start, end, dt): # Define the time points within a period (noninclusive to last time step)
        width = end - start
        sampled_time_steps = ceil(width / dt) - 1
        sampled_width_ms = sampled_time_steps * dt # Effective length of pulse when accounting for sampling resolution & excluding last time step
        npoints = sampled_time_steps + 1  # Number of time points within a pulse
        return np.linspace(start, start+sampled_width_ms, npoints) # Time points within a pulse starting at t=0

    # Define the time points within a pulse
    if pat.pulse_shape.shape == "Ideal_Square":
        pulse_t = np.linspace(0, pulse_duration_ms, 2) # Only need the first and last time point of the pulse, as it does not change over the duration
    else:
        # Concatenate the time points of the high resolution and low resolution periods
        pulse_t = np.concatenate([get_t(0, hi_res_width_ms, pulse_dt), get_t(hi_res_width_ms, pulse_duration_ms, default_dt)])
    # print(get_t(0, hi_res_width_ms, pulse_dt))
    # print(get_t(hi_res_width_ms, pulse_duration_ms, default_dt))
    # Get the pulse waveform
    pulse = efield_waveform(pulse_t) * efield_amplitude_mV_per_um # Sample points of the pulse waveform; scale by efield_amplitude_mV_per_um
    
    ## Build the time course of the electric field waveform
    # Initial silent period
    if stim_start_ms > 0:
        time.append(0)
        wav.append(0)

    # Write pulses into waveform
    pulse_end_time = 0
    for i, pulse_start in enumerate(pulse_start_times_ms):
        # Buffers necessary for interpolated simulation time steps when outside of a pulse
        # Pre-pulse buffer
        ...
        # Post-pulse buffer
        pulse_end_time = pulse_start + pulse_duration_ms # Time of the end of the pulse
    ...
    def NetPyNE_interval_func(t):
        ...
        prev_pulse_hi_res_ends = [end for end in pulse_hi_res_end_times_ms if end <= t]
        ind_last_pulse_hi_res_end = len(prev_pulse_hi_res_ends)-1

        if ind_last_pulse_hi_res_end == -1: # If there have not been any pulses yet
            last_pulse_hi_res_end = 0
        else:
            last_pulse_hi_res_end = pulse_hi_res_end_times_ms[ind_last_pulse_hi_res_end]

        if ind_last_pulse_hi_res_end + 1 < len(pulse_hi_res_end_times_ms):
            next_pulse_hi_res_end = pulse_hi_res_end_times_ms[ind_last_pulse_hi_res_end+1]
        else:
            next_pulse_hi_res_end = -1

        # Set h.dt
        within_hi_res_pulse = last_pulse_start >= last_pulse_hi_res_end # Within a high resolution period of a pulse if the last time a period started was more recent than the last time one ended
        if within_hi_res_pulse:
            h.dt = pulse_dt
        else:
            # if h.dt != default_dt:
            #     print("Switching to default dt at t=", t)
            h.dt = default_dt
        
        if next_pulse_start==-1:        # If there are no more pulse starts
            next_event=next_pulse_hi_res_end   # Next event must be a pulse end or there are no more events (next_event=-1)
        elif next_pulse_hi_res_end==-1:        # If there are no more pulse ends but there are more pulse starts
            next_event=next_pulse_hi_res_end   # Next event must be a pulse start
        else:                           # If there are both more pulse starts and ends
            next_event = min([next_pulse_start, next_pulse_hi_res_end])    # Next event is the one that comes sooner
...
def check_nonspecific_parameters(
    ...
    if num_time_steps_in_pulse_width != None:
        if num_time_steps_in_pulse_width <= 0:
            raise ValueError(f"num_time_steps_in_pulse_width [{num_time_steps_in_pulse_width}] must be > 0 or None")
        else:
            pulse_dt = min(pulse_dt, pulse_width_ms/num_time_steps_in_pulse_width)
...
def get_efield_sTMS(
        ...
        num_time_steps_in_pulse_width: int | None = None,
        plot: bool = False,
        **kwargs,
    ):
    """
    ...
    stim_end_ms, pulse_dt, pulse_width_ms = check_nonspecific_parameters(
            ...
            pulse_dt=pulse_dt,
            num_time_steps_in_pulse_width=num_time_steps_in_pulse_width,
        )
