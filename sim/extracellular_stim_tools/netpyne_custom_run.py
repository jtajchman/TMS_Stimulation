...
# ------------------------------------------------------------------------------
def runSimWithIntervalFunc(interval, func, func_first=False, timeRange=None, funcArgs=None):
    """
    ...
    if interval == 'dt':
        while round(h.t, 9) < stopTime:
            runFordt(func, func_first, **kwargs)
    else:
        while round(h.t, 9) < stopTime:
...
def runFordt(func, func_first, **kwargs):
    from netpyne import sim

    if func_first:
        func(h.t, **kwargs)  # function to be called at intervals

    sim.pc.psolve(h.t + h.dt)
